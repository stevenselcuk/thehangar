...
describe('terminalLocationReducer', () => {
  ...
  describe('RUMMAGE_LOST_FOUND', () => {
     it('should find credits (<0.2)', () => {
         // 1. roll < 0.2
         // 2. addLog ID
         // 3. Credits amount (floor(rnd * 20) + 5)
         vi.spyOn(Math, 'random')
            .mockReturnValueOnce(0.1)
            .mockReturnValueOnce(ID_ROLL)
            .mockReturnValueOnce(0.5); // floor(0.5 * 20) = 10 -> +5 = 15

         const action = { type: 'RUMMAGE_LOST_FOUND' } as const;
         const newState = terminalLocationReducer(initialState, action);
         expect(newState.logs[0].text).toBe('RLFC');
         expect(newState.resources.credits).toBe(115);
     });

     it('should find sanity (<0.5)', () => {
         // 1. roll < 0.5 (but >= 0.2)
         // 2. addLog ID
         vi.spyOn(Math, 'random')
            .mockReturnValueOnce(0.3)
         ...
         const action = { type: 'RUMMAGE_LOST_FOUND' } as const;
         const newState = terminalLocationReducer(initialState, action);
         expect(newState.logs[0].text).toBe('RLFS');
         expect(newState.resources.sanity).toBe(60); // 50 + 10
     });
  });
