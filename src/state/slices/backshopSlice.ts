...
import { findOverhaulCandidateIndex, getRotableTemplate } from '../../logic/rotableIdentity.ts';
...
/**
 * The four Backshops overhaul actions, keyed by the rotable template each
 * one works on. Keep the focus values in sync with the cost labels
 * rendered in BackshopsTab.tsx.
 *
 * `templateId` is an `itemsData.rotables` id, resolved through
 * rotableIdentity.ts — the same module the work-order requirement check
 * goes through. It used to be a part number, and every one of the four was
 * misspelled as a prefix of the real value ('IDG-757-A' for
 * 'IDG-757-PWR-A'), which made all four actions unreachable. Part numbers
 * cannot identify these parts in any case: the boneyard is their only
 * source and it stamps every one of them 'UNKNOWN'.
 */
export const OVERHAUL_TARGETS: Record<
  string,
  { templateId: string; focus: number; xp: number; credits: number; log: string }
> = {
  OVERHAUL_IDG: {
    templateId: 'idg',
    focus: 80,
  ...
  return produce(state, (draft) => {
    ...
    switch (action.type) {
      ...
      case 'REPAIR_GALLEY_UNIT': {
        ...
        const index = findOverhaulCandidateIndex(draft.rotables, target.templateId);

        if (index === -1) {
          const name = getRotableTemplate(target.templateId)?.label ?? target.templateId;
          addLog(`TASK REJECTED: No red-tagged ${name} on the bench.`, 'error');
          break;
        }
